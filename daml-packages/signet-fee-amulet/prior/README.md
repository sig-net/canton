...
When cutting a **new version** of `signet-fee-amulet`, place the **previously
released DAR** here so the build and CI can verify the new version is a valid
Smart Contract Upgrade of it.

## Release workflow

1. Bump `version:` in `../daml.yaml`. Keep `name:` **unchanged** — the package
   name is the SCU upgrade key; the `FeeCollector` interface instance and any
   `#signet-fee-amulet:...` refs rely on it staying stable across versions.
2. Copy the last released DAR into this directory:
   ```
   cp <released>/signet-fee-amulet-0.0.1.dar prior/
   ```
