...
When cutting a **new version** of `signet-signer-v1`, place the **previously released
...
1. Bump `version:` in `../daml.yaml`. Keep `name:` **unchanged** — the package
   name is the SCU upgrade key and the `#signet-signer-v1:Module:Tmpl` refs in the
   TS client and the MPC indexer rely on it staying stable across versions.
2. Copy the last released DAR into this directory:
   ```
   cp <released>/signet-signer-v1-0.0.1.dar prior/
   ```
