...
// ---------------------------------------------------------------------------
// Cross-language UInt256 vectors — oracle suite.
//
// Oracle pattern: TypeScript `BigInt` computes the expected value for every
// operation (add/sub/mul/div/compare + checked variants); the Daml suites assert
// byte-identical hex against these vectors. TS and Daml vectors MUST stay in sync
// — any change here must be mirrored in the Daml suites, and vice versa.
//
// Suite map (counts approximate):
// - this file: TS BigInt oracle (~2,392 cases).
// - ../../signet-uint256-tests/daml/TestUInt256.daml: 72 vector assertions.
// - ../../signet-uint256-tests/daml/TestFuzz.daml: 4 × 500 frozen fuzz vectors,
//   generated by scripts/generate-uint256-vectors.ts (seed keccak256("uint256-fuzz-vectors-v1")).
// - ../../signet-uint256-tests/daml/TestProperties.daml: 22 property tests (no oracle).
// - ../../signet-uint256-tests/daml/TestDivByZero.daml: div-by-zero error path.
//
// Vector provenance: overflow boundary pairs from holiman/uint256; the squaring
// sweep (2^k-1)^2 and Karatsuba-threshold cases from CPython test_long.py; EVM
// uint256 vectors from parity-common `uint`.
// ---------------------------------------------------------------------------
