...
- the **`signet-signer-v1` DAR** loaded into it, and a freshly created `Signer` contract (co-signed by `SigNetwork` + `SigNetworkFA` via `SignerProposal` → `AcceptSigner`) + parties (`SigNetwork` / `SigNetworkFA` / `Operator` / `Requester`);
...
The harness loads a checked-in copy of this repo's `signet-signer-v1` DAR (`integration-tests/fixtures/canton/signet-signer-v1-0.0.1.dar`). After changing Daml here, rebuild and copy it over:
