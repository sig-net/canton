/**
 * Deploy the fee-enabled MPC stack to a Canton network — free mode (feeAmount = 0) by
 * default, or PAID mode when MPC_CANTON_FEE_AMOUNT > 0 (a real CC transfer is charged).
 *
 * Network is selected by CANTON_NETWORK (default "devnet"); load that network's inputs
 * with DOTENV_CONFIG_PATH=.env.<network>. Outputs route per network: ids into
 * test/.env[.<network>] and disclosures into apps/disclosure-api/disclosures.<network>.ts.
 *
 * Creates, as the single network party (operators = requester = sigNetwork = fee
 ...
 * writes the resulting ids into the network's .env (backing up <file>.bak first).
 *
 *   # DevNet (default network): reads test/.env
 *   pnpm exec tsx src/scripts/deploy.ts                                  # dry-run (free)
 *   DEPLOY_CONFIRM=1 pnpm exec tsx src/scripts/deploy.ts                 # deploy (free)
 *   DEPLOY_CONFIRM=1 MPC_CANTON_FEE_AMOUNT=0.01 pnpm exec tsx src/scripts/deploy.ts  # paid
 *
 *   # Testnet / mainnet: select the network + load its env file for inputs
 *   CANTON_NETWORK=testnet DOTENV_CONFIG_PATH=.env.testnet \
 *     DEPLOY_CONFIRM=1 pnpm exec tsx src/scripts/deploy.ts              # deploy to testnet (free)
 */
...
// Target network: selects which `.env.<network>` + `disclosures.<network>.ts` the run
// writes. Defaults to "devnet" (legacy behavior: writes test/.env + disclosures.devnet.ts).
// For other networks (e.g. CANTON_NETWORK=testnet) load that network's inputs with
// DOTENV_CONFIG_PATH=.env.<network>; outputs are routed to the matching files.
const NETWORK = process.env.CANTON_NETWORK ?? "devnet";
const ENV_SUFFIX = NETWORK === "devnet" ? "" : `.${NETWORK}`;
...
async function main(): Promise<void> {
  ...
  if (!CONFIRM) {
    ...
    console.log(
      `[deploy]               FeePriceConfig(feeAmount=${WANT_PAID ? FEE_AMOUNT : "0.0"}), Vault — then write ids to test/.env${ENV_SUFFIX}.`,
    );
    console.log(
      `[deploy] Network:   ${NETWORK} (writes .env${ENV_SUFFIX} + disclosures.${NETWORK}.ts)`,
    );
    return;
  }
  ...
  const discPath = fileURLToPath(
    new URL(`../../../apps/disclosure-api/disclosures.${NETWORK}.ts`, import.meta.url),
  );
  ...
  console.log(
    `[deploy] Wrote disclosures (DisclosedContract shape) to apps/disclosure-api/disclosures.${NETWORK}.ts`,
  );

  // 7. Persist ids into test/.env (back up first; secret lines are left untouched).
  const envPath = fileURLToPath(new URL(`../../.env${ENV_SUFFIX}`, import.meta.url));
  upsertEnv(envPath, {
  ...
  console.log(`[deploy] Wrote ids to test/.env${ENV_SUFFIX} (backup: test/.env${ENV_SUFFIX}.bak).`);
}
